...
def plot_roc_curve(est, x_test, y_test, path):
    clf, name = est
    fig_index = 1
    ...
    ax1.plot([0, 1], [0, 1], "k:", label="Luck")
    ax1.plot(fpr, tpr, "s-", label="%s (%1.3f)" % (name, clf_score))
...
def plot_calibration_curve(est_group, x_test, y_test, path):
    fig_index = 1
