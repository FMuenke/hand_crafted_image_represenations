...
from handcrafted_image_representations.machine_learning.feature_map import FeatureMap
...
class LeungMalik:
    ...
    def _collect_descriptors(self, list_of_targets, key_points):
        ...
        for target in list_of_targets:
            ...
            dc = f_map.to_descriptor_with_pooling(key_points, pooling_mode="mean")
            if dc is not None:
                dc_sets.append(dc)
        return dc_sets
    ...
    def compute(self, image, key_points):
        ...
        dc_sets = self._collect_descriptors(feature_tensors, key_points)
        if len(dc_sets) == 0:
            return None
        elif len(dc_sets) == 1:
            return dc_sets[0]
